...
import it.neckar.open.javafx.Components.checkBox
import it.neckar.open.javafx.Components.comboBox
import it.neckar.open.javafx.Components.hbox5
import it.neckar.open.javafx.Components.headline1
import it.neckar.open.javafx.Components.label
import it.neckar.open.javafx.Components.slider
...
class ChartingDemosFX : Application() {
  ...
  override fun start(primaryStage: Stage) {
    ...
    vbox5(hbox5(filterText, button("Clr") {
      filterText.text = ""
    }), button("Exp") {
      treeView.root.children.forEach { treeItem ->
  ...
  private fun createConfigurationPane(meisterChart: MeisterChartFX, demo: ChartingDemo, canvasNodeProvider: () -> MeisterChartFX): Node {
    ...
    pane.add(headline1("Configuration"), "span")

    pane.add(label("Zoom"))
    pane.add(label(layerSupport.chartSupport.rootChartState.zoomProperty.toJavaFx().map { it.format() }), "span")
    pane.add(label("Translation"))
    pane.add(label(layerSupport.chartSupport.rootChartState.windowTranslationProperty.toJavaFx().map { it.format() }), "span")

    pane.add(label("Window Size"))
    pane.add(label(layerSupport.chartSupport.rootChartState.windowSizeProperty.toJavaFx().map { it.format() }), "split 2, span")
    pane.add(label(layerSupport.chartSupport.canvas.chartSizeClassificationProperty.toJavaFx().map { "${it.width.name} / ${it.height.name}" }), "")

    pane.add(label("Content Area Size"))
    pane.add(label(layerSupport.chartSupport.rootChartState.contentAreaSizeProperty.toJavaFx().map { it.format() }), "span")

    pane.add(label("Y Axis Orientation"))
    pane.add(hbox5(comboBox(layerSupport.chartSupport.rootChartState.axisOrientationYProperty.toJavaFx(), AxisOrientationY.values()).also { it.cellFactory }), "span")

    pane.add(label("X Axis Orientation"))
    pane.add(hbox5(comboBox(layerSupport.chartSupport.rootChartState.axisOrientationXProperty.toJavaFx(), AxisOrientationX.values())), "span")

    pane.add(label("Snap Configuration"))
    pane.add(hbox5(comboBox(layerSupport.chartSupport.pixelSnapSupport.snapConfigurationProperty.toJavaFx(), enumValues())), "span")

    pane.add(label("Global Tiles Cache"))
    pane.add(label(
    ...
    pane.add(button("Clear Global Tiles Cache") {
      GlobalTilesCache.clearAll()
    }, "span")

    pane.add(label("Background"))
    ...
    pane.add(hbox5(comboBox(backgroundProperty, backgroundColors) { "$it" }), "span")
    ...
    pane.add(label("Corporate Design"))
    ...
    val comboBox = comboBox(corporateDesignConfig.toJavaFx(), listOf(NeckarITDesign, SegoeUiDesign, DebugDesign)) { it.id }
    pane.add(comboBox, "span")

    pane.add(label("Device Pixel Ratio"))
    ...
    val slider = slider(devicePixelRatioProperty.toJavaFx(), 0.25, 2.0, 0.1).also {
      it.isSnapToTicks = true
      it.isShowTickLabels = true
      it.isShowTickMarks = true
    }
    pane.add(hbox5(slider, label(devicePixelRatioProperty.map { decimalFormat.format(it) }.toJavaFx())), "span")

    pane.add(button("Set to 1.0") {
      devicePixelRatioProperty.value = 1.0
      layerSupport.markAsDirty()
    }, "skip 1, split 2, span")

    val initialMainScreenDevicePixelRatio = mainScreenDevicePixelRatio
    pane.add(button("Set to Device Pixel Ratio") {
      devicePixelRatioProperty.value = initialMainScreenDevicePixelRatio
      layerSupport.markAsDirty()
    }, "span")

    pane.add(label("Text Locale"))
    pane.add(hbox5(comboBox(textLocale.toJavaFx(), listOf(Locale.US, Locale.Germany, Locale("hu-HU"))) {
      it.locale
    }), "span")

    pane.add(label("Format Locale"))
    pane.add(hbox5(comboBox(formatLocale.toJavaFx(), listOf(Locale.US, Locale.Germany, Locale("hu-HU"))) {
      it.locale
    }), "span")

    pane.add(label("Time zone"))
    pane.add(hbox5(comboBox(timeZone.toJavaFx(), listOf(TimeZone.UTC, TimeZone.Berlin, TimeZone.Tokyo, TimeZone("America/Chicago"))) {
      it.zoneId
    }), "span")

    pane.add(headline1("Debug Features"), "span, gaptop 15px")
    ...
    DebugFeature.values().map { debugFeature ->
      ...
      val checkBox = checkBox(debugFeature.name, debugModeEnabled.toJavaFx())
    ...
    pane.add(button("Toggle All") {
      chartSupport.debug.toggle()
    }, "span")


    pane.add(headline1("Debug Tools"), "span, gaptop 15px")

    pane.add(button("Open Debug View") { _ ->
    ...
    pane.add(button("Repaint") {
      layerSupport.markAsDirty()
    }, "span")

    pane.add(button("Dump Layers") { _ ->
      logger.info("Layers:")
    ...
    pane.add(button("Dump Cache State") {
      printCacheReport()
    }, "span")

    pane.add(button("Screenshot") {
    ...
    pane.add(checkBox("Performance Layer", repaintPerformanceLayerVisible.toJavaFx()), "span")
    ...
    pane.add(checkBox("Always Repaint", forceRepaint.toJavaFx()), "span")
    ...
    pane.add(checkBox("Content Area Debug", contentAreaDebugLayerVisible.toJavaFx()), "span")
    ...
    pane.add(checkBox("Content Viewport Debug", contentViewportDebugLayerVisible.toJavaFx()), "span")
    ...
    pane.add(checkBox("Whats At Debug", whatsAtDebugLayerVisible.toJavaFx()), "span")
    ...
    pane.add(checkBox("Window Debug", windowDebugLayerVisible.toJavaFx()), "span")
    ...
    pane.add(checkBox("PaintingVariables Debug", paintingVariablesDebugLayerVisible.toJavaFx()), "span")
  ...
  private fun createNavigation(): TreeView<TreeObject> {
    val byCategory = getDemoDescriptors().toMutableList().groupBy {
      it.category
    }.toSortedMap()
  ...
  @CalledOnce
  private fun getDemoDescriptors(): List<ChartingDemoDescriptor<*>> {
    val descriptors = mutableListOf<ChartingDemoDescriptor<*>>()
    descriptors.addAll(DemoDescriptors.descriptors)
    descriptors.addAll(DemoDescriptorsFx.descriptors)
    return descriptors
  }
