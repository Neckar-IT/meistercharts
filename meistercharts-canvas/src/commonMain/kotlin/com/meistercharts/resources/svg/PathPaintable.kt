...
fun Path.toProvider(
   ...
   * The default fill
   */
  defaultFill: ColorProviderNullable = Palette.defaultGray,
...
class PathPaintableProvider(
   ...
   */
  val defaultFill: ColorProviderNullable,

  /**
   * The default stroke that is used if no fill is provided in the [get] method
   */
  val defaultStroke: ColorProviderNullable = null,
) {
  /**
   * Returns the paintable for the given size and with the given fill
   */
  fun get(targetSize: Size, fill: ColorProviderNullable = defaultFill, stroke: ColorProviderNullable = defaultStroke, strokeWidth: @px Double = 1.0, alignmentPoint: Coordinates = Coordinates.origin): PathPaintable {
   ...
   */
  fun get(targetSize: Size, fill: ColorProviderNullable = defaultFill, stroke: ColorProviderNullable = defaultStroke, alignment: Direction): PathPaintable {
...
class PathPaintable(
   ...
   */
  fun withFill(fill: ColorProviderNullable): PathPaintable {
    return PathPaintable(pathActions, size, scale, offset, fill, stroke, strokeWidth, alignmentPoint)
  }

  fun withStroke(stroke: ColorProviderNullable): PathPaintable {
    return PathPaintable(pathActions, size, scale, offset, fill, stroke, strokeWidth, alignmentPoint)
  }
