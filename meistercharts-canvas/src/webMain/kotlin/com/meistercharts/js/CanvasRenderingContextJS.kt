...
class CanvasRenderingContextJS(
   ...
   */
  private fun CanvasPaint.toHtml(): JsAny {
    return when (this) {
      is Color -> web.toJsString()
...
fun HTMLCanvasElement.getCanvasRenderingContext2D(readBackFrequency: CanvasReadBackFrequency): CanvasRenderingContext2D {
  val willReadFrequently = when (readBackFrequency) {
    CanvasReadBackFrequency.Frequent -> true
    CanvasReadBackFrequency.Infrequent -> false
  }

  return getContext2d(willReadFrequently) ?: throw IllegalStateException("context not found")
}

/**
 * Calls `getContext("2d", {willReadFrequently: ...})` - the options object literal requires target-specific interop.
 */
internal expect fun HTMLCanvasElement.getContext2d(willReadFrequently: Boolean): CanvasRenderingContext2D?
